...
class Devices extends Component {
  ...
  componentDidMount() {
    const activitiesUrl = "http://localhost:8282/hubs/harmony-hub/devices";
    fetch(activitiesUrl)
      .then(response => response.json())
      .then(data => {
        this.setState({ activities: data.devices });
      })
  }
  ...
  onStartActivity(activity) {
    ...
    const activitiesPostUrl = `http://localhost:8282/hubs/harmony-hub/devices/${activity.slug}`;
